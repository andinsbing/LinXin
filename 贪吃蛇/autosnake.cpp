...
{
	Decision decision = breadthFisrtSearch();
	_map->removeGameItem(_snake.tail());
	switch (decision)
	{
	case AutoSnake::none: // if no decision, rand move
		_snake.randMove();
		break;
	case AutoSnake::toLeft:
		_snake.left();
		break;
	case AutoSnake::toRight: 
		_snake.right();
		break;
	case AutoSnake::toUp: 
		_snake.up();
		break;
	case AutoSnake::toDown: 
		_snake.down();
		break;
	default:
...
AutoSnake::Decision AutoSnake::breadthFisrtSearch()
{
	enum Flag
	{
